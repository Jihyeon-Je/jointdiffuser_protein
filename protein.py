...
def read_pdb(pdb_file: str, chain_id: Optional[str] = None) -> Protein:
    ...
    for chain in model:
        ...
        for res in chain:
            ...
            num_atoms = len([atom for atom in res])
            """
            try:
                assert num_atoms == len(residue_constants.residue_atoms[res.get_resname()])
            except AssertionError:
            if num_atoms != len(residue_constants.residue_atoms[res.get_resname()]):
                print(f"residue {res.get_resname()} has {num_atoms - len(residue_constants.residue_atoms[res.get_resname()])} atoms")
                print(f"residue {res.get_resname()} has {num_atoms} atoms")
                print(f"residue {res.get_resname()} has {len(residue_constants.residue_atoms[res.get_resname()])} atoms")
            print(f"residue {res.get_resname()} has {num_atoms - len(residue_constants.residue_atoms[res.get_resname()])} atoms")
            """
            
            #^ 이게 당연히 일치 하지 않지, H atom들이 있으니까. 근데 pdb file 중에 H atom들이 없는 경우도 있는 것 같다. 그래서
            #^ 같은 residue에 대해서 atom의 개수가 다른 경우가 있음. 그래서 이 부분은 일단 무시하고 넘어가자.
            for atom in res:
