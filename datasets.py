...
class LigProtDatabase(Dataset):
    ...
    def __getitem__(self, index):
        ...
        p_feats = load_feats_from_pdb(self.protpaths[index])
        bb_coords = p_feats['bb_coords']
        protein = einops.rearrange(bb_coords, 'h w c -> c h w')
        
        #print(self.ligpaths[index])
        suppl = Chem.SDMolSupplier(self.ligpaths[index], sanitize=False)
        try:
...
class LigProtFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        print(os.path.join(self.root, f'{index}.npy'))
        z, c = np.load(os.path.join(self.root, f'{index}.npy'), allow_pickle = True)
        return z, c
    
    def __len__(self):
        files = glob.glob(os.path.join(self.root, '*.npy'))
        return len(files)
...
def get_dataset(name, **kwargs):
    ...
    elif name == 'ligprot_features':
        return LigProtFeatures(**kwargs)
    else:
